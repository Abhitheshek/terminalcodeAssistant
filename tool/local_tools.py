...
def run_pytest(test_path: str = ".") -> str:
    """
    Run pytest on specified path with verbose output.
...
@tool
def scan_project(directory: str = ".", max_depth: int = 5) -> str:
    """
    Recursively scan entire project structure with all files and folders.
    ...
    try:
        ...
        def build_tree(path, prefix="", depth=0):
            ...
            try:
                ...
                dirs = [e for e in entries if os.path.isdir(os.path.join(path, e)) and e not in ignore_dirs and not e.startswith('.')]
                files = [e for e in entries if os.path.isfile(os.path.join(path, e)) and not e.startswith('.') and not any(e.endswith(ext) for ext in ignore_files)]
                
                for i, d in enumerate(dirs):
                    is_last = (i == len(dirs) - 1) and len(files) == 0
                    items.append(f"{prefix}{'└── ' if is_last else '├── '}📁 {d}/")
                    new_prefix = prefix + ('    ' if is_last else '│   ')
                    items.extend(build_tree(os.path.join(path, d), new_prefix, depth + 1))
                
                for i, f in enumerate(files):
                    is_last = i == len(files) - 1
                    size = os.path.getsize(os.path.join(path, f))
                    items.append(f"{prefix}{'└── ' if is_last else '├── '}📄 {f} ({size}b)")
            except PermissionError:
                items.append(f"{prefix}[Permission Denied]")
            
            return items
        
        tree = [f"📁 {os.path.basename(directory)}/"] + build_tree(directory)
        return "\n".join(tree)
...
def read_multiple_files(file_paths: str) -> str:
    """
    Read multiple files at once. Useful for batch reading.
